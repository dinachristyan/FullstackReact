...
function DataTrainer() {
  ...
  const fectData = async () => {
    //fetching
    const response = await axios.get("https://localhost:7190/api/Trainer");
    //get response data
  ...
  const columns = [
    ...
    {
      name: 'Ubaah',
      selector: row => <Link to={"/datatrainer_edit/"+row.id}
      ...
      {
      name: 'Hapus',
      selector: row => <Link to={"/datatrainer_delete/"+row.id}
